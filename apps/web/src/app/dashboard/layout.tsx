...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-svh w-full">
      <Sidebar accounts={slimAccounts} categories={slimCategories} baseCurrency={profile.base_currency} />
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col lg:max-w-4xl">
        ...
        <BottomNav accounts={slimAccounts} categories={slimCategories} baseCurrency={profile.base_currency} />
      </div>
    </div>
  );
}
